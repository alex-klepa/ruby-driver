    ...
    class Connector
      ...
      def do_connect(host)
        @reactor.connect(host.ip.to_s, @connection_options.port, {:timeout => @connection_options.connect_timeout, :ssl => @connection_options.ssl}) do |connection|
          ...
          Protocol::CqlProtocolHandler.new(connection, @logger, @reactor, @connection_options.protocol_version, @connection_options.compressor, @connection_options.heartbeat_interval, @connection_options.idle_timeout)
